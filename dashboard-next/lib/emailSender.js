...
export function getAvailableAccounts(extraAccounts = []) {
  ...
  const accounts = [];
  const graphAccount = buildGraphAccount();
  const defaultAccount = buildDefaultAccount();

  if ((provider === 'graph' || provider === 'auto') && graphAccount) {
    accounts.push(graphAccount);
  }
  if ((provider === 'smtp' || provider === 'auto') && defaultAccount.host && defaultAccount.user && defaultAccount.pass && defaultAccount.from) {
    accounts.push(defaultAccount);
  }

  for (const acct of extraAccounts || []) {
    if (acct?.oauthAccountId && acct?.from) {
      accounts.push(acct);
...
async function getDelegatedAccessToken(oauthAccountId) {
  ...
  if (doc.accessTokenEnc && expiresAtMs > now + 60000) {
    return decryptString(doc.accessTokenEnc);
  }

  const refreshToken = decryptString(doc.refreshTokenEnc);
  const scope = doc.scopes && doc.scopes.length ? doc.scopes.join(' ') : ['offline_access', 'User.Read', 'Mail.Send'].join(' ');
...
export async function verifyAccountConnection(account) {
  ...
  if (account.provider === 'graph') {
    ...
    return {
      ok: true,
      message: 'Graph (app) account connected'
    };
  }
  ...
  if (account.provider === 'graph_oauth') {
    ...
    return {
      ok: true,
      message: 'Graph OAuth account connected'
    };
  }
